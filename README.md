...
Battery SipJuice 是一个面向 Linux 笔记本和平板的本机电池监控与电源状态工具。它使用 Tauri v2 + Rust + 原生 Web 前端构建，主要从 Linux `sysfs` / `procfs` 读取电池、电源输入、CPU/GPU 频率和进程 CPU 时间数据。
...
- **超级省电模式**：通过 Linux `cpufreq` / `devfreq` 限制 CPU/GPU 最高频率，降低峰值功耗和发热。
...
- **超级省电需要管理员权限**：写入 `cpufreq` / `devfreq` 上限通常需要 `pkexec` 提权。
...
- 历史数据使用固定大小的 RRD 风格二进制归档，避免历史文件无限增长。
...
- 补充不同 Linux 设备上的 `power_supply` / `cpufreq` / `devfreq` 兼容性信息。
- 改进 Intel / AMD / NVIDIA GPU 频率读取。
