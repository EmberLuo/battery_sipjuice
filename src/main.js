...
import { currentLanguage, hasLanguage, setLanguage, translate } from "./i18n.js";
import { formatDuration, formatNumber, formatValueWithUnit } from "./formatters.js";
import { refreshBatteryInsights, renderBatteryInsights } from "./insights.js";
...
byId("batterySelect").addEventListener("change", async (event) => {
  ...
  await refreshBatteryInsights(selectedBattery());
});
...
document.querySelectorAll(".tab").forEach((tab) => {
  ...
  if (tab.dataset.tab === "sessions" || tab.dataset.tab === "health") {
    tab.addEventListener("click", () => refreshBatteryInsights(selectedBattery()));
  }
});
...
async function tick() {
  try {
    ...
    if (batteryChanged && selectedBatteryId) {
      ...
      refreshBatteryInsights(selectedBattery());
    }
    renderInputSourceOptions();
...
setInterval(() => refreshBatteryInsights(selectedBattery()), 30_000);
...
async function loadSettings() {
  try {
    ...
    settings.language = hasLanguage(settings.language) ? settings.language : "zh-CN";
